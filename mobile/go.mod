// Ядро мобильной версии: один Go-модуль, из которого gomobile собирает ОДИН
// .aar для приложения.
//
// Почему один, а не два отдельных (libbox от sing-box + обёртка над Xray):
// gomobile кладёт в каждый .aar свою `lib/<abi>/libgojni.so` под одним и тем
// же именем. Две библиотеки в одном APK по этому имени конфликтуют, и вторая
// молча не попадает в сборку. Поэтому оба пакета передаются в ОДНУ команду
// `gomobile bind`, и рантайм Go в приложении ровно один.
//
// Зачем вообще Xray, когда есть sing-box: sing-box не понимает транспорт
// xhttp вообще (проверено по его исходникам — ни одного упоминания
// xhttp/splithttp). В подписке пользователя на нём 5 серверов из 15. На
// Windows это решено мостами: отдельный процесс Xray на сервер, SOCKS5 на
// локальном порту. На Android процессов нет, но схема переносится один в
// один — мосты поднимаются внутри того же процесса приложения.
module github.com/MrMultik/Multik-Sila/mobile
