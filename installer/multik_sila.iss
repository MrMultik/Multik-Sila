...
WizardStyle=modern
; Изображения мастера — заводские. Свои тут пробовали и убрали: боковая
; картинка видна один раз при установке, а иконка в заголовке окна (и у
; самого setup.exe) — постоянно, и менять имело смысл именно её.
;
; Иконка установщика. Без неё Inno ставит свою — коробку со стрелкой вниз.
...
// %APPDATA%\com.example\Multik Sila (по 1.0.10 включительно —
// %APPDATA%\com.example\proxy_app_test, её приложение при первом запуске
// копирует в новую и оставляет как есть) — деинсталлятор ни в одну из них
// не заходит, так что подписка переживает удаление.
