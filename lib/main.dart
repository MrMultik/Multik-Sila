...
/// Внутренние пространства имён: они существуют только внутри локальной или
/// корпоративной сети, и в интернете их нет по определению. Отправлять их в
/// туннель бессмысленно вдвойне — прокси-сервер в другой стране не знает и не
/// может знать, что такое `mail.contoso.local`.
///
/// **Как это выходило наружу.** Под TUN мы перехватываем ВЕСЬ DNS, поэтому
/// такое имя доезжало до последнего правила и получало адрес FakeIP
/// (`198.18.0.0/15`). Приватным он не считается, правило `ip_is_private` его
/// не ловит — и соединение уезжало в туннель. В логе живой машины это видно
/// прямо: `outbound/socks[srv_2]: outbound connection to mail.rosatom.local:443`
/// от `OUTLOOK.EXE`, снова и снова, каждые пару секунд.
///
/// `.local` вдобавок зарезервирован под mDNS (RFC 6762) — его вообще нельзя
/// разрешать обычным DNS, тем более чужим.
const List<String> kPrivateDomainSuffixes = [
  '.local',
  '.localdomain',
  '.lan',
  '.home',
...
class MyApp extends StatelessWidget {
  ...
  ThemeData _theme(Color seed, Brightness brightness) {
    ...
    return ThemeData(
      useMaterial3: true,
      colorScheme: scheme,
      // Запасной шрифт — ради флагов стран. Системный шрифт Windows их не
      // содержит, и вместо 🇩🇪 в имени сервера рисуются буквы «DE» (см.
...
''';
  ...
  List<ParsedServer> _autoSelectCandidates() {
    ...
    // Серверы, которые проверка связи уже признала мёртвыми, в автовыбор не
    // берём. Без этого получался круг: проверка уводит с нерабочего сервера,
    // а автовыбор через пятнадцать минут возвращает обратно — он сортирует
    // ТОЛЬКО по задержке, а быстрый пинг и рабочий сервер это разные вещи.
    //
    // Ровно так и выглядела беда: сервер с самым низким пингом во всей
    // подписке (trojan поверх gRPC) не проводил ничего вообще — в списке
    // соединений его TCP-сессии висели по девять минут с нулём байт в ОБЕ
    // стороны. Пока автовыбор стоял на нём, у человека не открывались ни
    // почта, ни сайты, и каждый раз это выглядело новой поломкой.
    //
    // Если мёртвыми помечены ВСЕ — метки игнорируем и берём полный список:
    // причина тогда общая (упала сеть), и остаться вовсе без кандидатов хуже,
    // чем выбрать сомнительного. Та же логика, что в _switchAwayFromUnhealthy.
    final healthy = list.where((s) => !_unhealthy.contains(s.name)).toList();
    if (healthy.isNotEmpty && healthy.length != list.length) {
      _appendLog(tp('log.autoSelectSkipBad', {'n': list.length - healthy.length}));
    }
    if (healthy.isNotEmpty) list = healthy;
  ...
  Future<bool> _probeActiveServer() async {
    ...
    if (_runningEngine == 'singbox') {
      try {
        ...
        // ровно то, что мы ловим.
        //
        // ЗДЕСЬ БЫЛА ещё и проверка резолва имён, и её пришлось убрать.
        //
        // Замысел был верный: сервер может возить веб и не разрешать имена
        // (FakeIP отвечает на A сам, поэтому браузер жив, а почта и домены
        // обхода — нет), и такой сервер проверка считала здоровым. Разбор
        // это подтвердил: сломанный сервер давал 0 разрешений из 4 против
        // 4 из 4 у соседнего.
        //
        // Но в работе проверка стала давать ЛОЖНЫЕ отказы, и цена оказалась
        // выше пользы. Из живого лога: за пять минут подряд забракованы
        // VLESS-Multik, Vless TLS, VLESS XR и VLESS X — то есть ВСЕ, — и
        // приложение пошло по кругу, меняя сервер каждые полторы минуты и
        // обрывая соединения. Ручная проверка тех же серверов в ту же минуту
        // проходила за 397 мс. Скорее всего дело в самом запросе: TXT на
        // случайное имя бьёт по авторитативным серверам, и те его придерживают
        // при повторах — то есть проверка ломается именно от того, что её
        // делают часто.
        //
        // Возвращать её можно только с доказательством, что ложных отказов
        // нет ПРИ РЕГУЛЯРНОМ повторении, а не в одиночном замере.
  ...
  Widget _shieldBlock(double available) {
    ...
    return Column(
      children: [
        ...
        // Абзац про охват обычного режима с главного экрана убран по просьбе
        // автора. Он появился не просто так — обычный режим подменяет
        // СИСТЕМНЫЙ ПРОКСИ, а читают его не все программы, и «Защищено» под
        // зелёным щитом когда-то стоило дня разбора, — но постоянная стена
        // текста под щитом мешает больше, чем помогает. Сам текст остался
        // (`home.proxyModeCoverage`) и по-прежнему объясняет режим на вкладке
        // маршрутизации; здесь его быть не должно.
