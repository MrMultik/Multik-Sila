...
- **Соединение, которое чинится само:** проверка связи замечает сервер, переставший
  пропускать трафик: в режиме «Авто» приложение уходит на другой, в ручном — сообщает
  об этом и оставляет выбор вам; после сна приложение ждёт сеть, а не винит серверы.
- **Готово к серверам на Xray 26.9.9+:** REALITY-серверы идут через Xray, поэтому
  продолжают работать и после перехода сервера на постквантовое рукопожатие, которое
  клиенты на sing-box не умеют.
...
В Multik Sila нет телеметрии, аналитики и учётных записей. Подписки и настройки
остаются на вашем устройстве. Само приложение обращается только:

- **к адресу вашей подписки** — чтобы загрузить и обновить серверы;
- **к серверам, к которым вы подключаетесь**;
- **к GitHub** — проверить обновления приложения и движков и освежить наборы правил
  маршрутизации (проверку обновлений можно выключить в **Настройки → Обновления**);
- **к адресу проверки задержки**, по умолчанию `cp.cloudflare.com` — замерить серверы и
  убедиться, что трафик проходит (меняется в **Настройки → Тест задержки**);
- **к DNS-серверам**, по умолчанию `1.1.1.1` и `8.8.8.8` (меняются в **Настройки → DNS**);
- только когда вы запускаете их из диагностики: к `api.ipify.org`, чтобы показать ваш
  внешний IP, и к `speed.cloudflare.com` для теста скорости; к NTP-серверу — только если
  включена сверка времени.
...
sing-box умеет TUN сам и делает основную работу. Два вида серверов идут через Xray:

- **`xhttp`** — этот транспорт sing-box не поддерживает вовсе;
- **REALITY** — начиная с Xray 26.9.9 REALITY-сервер отвергает рукопожатие без
  постквантового обмена ключами X25519MLKEM768, а sing-box 1.14 не отправляет его ни с
  одним из своих отпечатков. Клиент Xray отправляет — и со старыми серверами тоже работает.

В обычном режиме Xray сам поднимает локальный прокси, а в TUN и на Android каждому
такому серверу выделяется свой мост Xray, а маршрутизацию по-прежнему ведёт sing-box.
